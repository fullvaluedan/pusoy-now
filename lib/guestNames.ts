// Mirror of server/src/guest.ts - keep in sync.
//
// The server package is deliberately outside the Metro bundle, so the client
// cannot import server/src/guest.ts directly. This file duplicates the word
// lists and generator verbatim so the client can generate a local guest name
// on-device (lib/guest.ts) that has the same shape as the name better-auth's
// `generateName` hook writes into `user.name` server-side.
//
// Guest username generator: Reddit-style "AdjectiveNoun-NNNN" names for
// anonymous sessions (e.g. "SwiftNarwhal-4821"). Words are family-friendly and
// deliberately kept clear of anything profanity-adjacent (no pairing produces
// a slur or an offensive phrase); both are Capitalized so the concatenation
// reads cleanly.
